...
function sanitizeSnapshot(snapshot: DaemonSettingsSnapshot): DaemonSettingsSnapshot {
  return {
    ...snapshot,
    desired: {
      ...snapshot.desired,
...
export function settingsRoutes(
  ...
  app.get("/settings/daemon", async () => {
    return sanitizeSnapshot(await daemonSettingsService.getSnapshot());
  });

  app.patch<{ Body: DaemonSettingsPatch }>("/settings/daemon", async (req, reply) => {
    try {
      return sanitizeSnapshot(await daemonSettingsService.update(req.body ?? {}));
    } catch (err) {
